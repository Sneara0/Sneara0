## 🌟 A Passionate Frontend Developer from Bangladesh 🇧🇩

<p align="center">
  <img src="https://avatars.githubusercontent.com/u/your_user_id?v=4" width="150" style="border-radius:50%; box-shadow: 0 0 25px #00CFFF; transition: 0.3s;" />
</p>

![Typing Effect](https://readme-typing-svg.demolab.com?font=Fira+Code&size=30&duration=4000&pause=1000&color=00CFFF,FF00FF&vCenter=true&width=650&lines=Hi+I'm+Sneara+Parvin;Frontend+Developer+💻;Let's+Build+Awesome+Web+Apps)

![Profile Views](https://komarev.com/ghpvc/?username=Sneara0&label=Profile%20views&color=0e75b6&style=flat)

---

## 🎉 Animated Badges (Auto Dark/Light)

<p align="left">
  <img src="https://img.shields.io/badge/Frontend_Developer-💻-brightgreen?style=for-the-badge&logoColor=white&labelColor=2F3136" />
  <img src="https://img.shields.io/badge/React-⚛️-blue?style=for-the-badge&logoColor=white&labelColor=2F3136" />
  <img src="https://img.shields.io/badge/JavaScript-ES6+-yellow?style=for-the-badge&logoColor=white&labelColor=2F3136" />
  <img src="https://img.shields.io/badge/TailwindCSS-38BDF8?style=for-the-badge&logoColor=white&labelColor=2F3136" />
  <img src="https://img.shields.io/badge/Love_Coding-❤️-red?style=for-the-badge&logoColor=white&labelColor=2F3136" />
  <img src="https://img.shields.io/badge/Currently_Learning-Node.js-blue?style=for-the-badge&logoColor=white&labelColor=2F3136" />
  <img src="https://img.shields.io/badge/Open_to_Opportunities-💼-success?style=for-the-badge&logoColor=white&labelColor=2F3136" />
</p>

---

## 🌗 Dark / Light Mode Toggle Preview

<p align="center">
  <img src="https://github.com/kshitij978/github-readme-dark-light-mode/raw/master/assets/dark-light-mode.gif" width="600" />
</p>

---

### 🚀 About Me

I am a passionate **Frontend Developer** from Bangladesh 🇧🇩 with hands-on experience in building **responsive, modern, and interactive** web applications using **React.js, JavaScript, and Tailwind CSS**.

🌱 Currently learning **Node.js, Express.js, MongoDB** to become a **Full Stack Developer**  
⚡ I love beautiful UI, clean code & coffee ☕  
🎯 Portfolio → [portfolio-q-plum.vercel.app](https://portfolio-q-plum.vercel.app/)

---

### 🌐 Connect with Me (Hover Effects)

<p align="left">
  <a href="https://www.linkedin.com/in/sneara-parvin-aa0a4b285/" title="LinkedIn" target="_blank"><img src="https://skillicons.dev/icons?i=linkedin" width="40" style="transition: transform 0.3s;" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'" /></a>
  <a href="https://www.facebook.com/sneyara.parabhina" title="Facebook" target="_blank"><img src="https://skillicons.dev/icons?i=facebook" width="40" style="transition: transform 0.3s;" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'" /></a>
  <a href="mailto:snearaparvin.cse@gmail.com" title="Gmail" target="_blank"><img src="https://skillicons.dev/icons?i=gmail" width="40" style="transition: transform 0.3s;" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'" /></a>
</p>

---

## 🏆 GitHub Profile Trophy

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=Sneara0&theme=radical&no-frame=true&row=1&column=6" />
</p>

---

### 🛠️ Technologies

#### 🎨 Frontend
<p>
  <img src="https://skillicons.dev/icons?i=react,html,css,tailwind,js,ts" />
</p>

#### ⚙️ Backend
<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express,python,django,nextjs" />
</p>

#### 🗄️ Database
<p>
  <img src="https://skillicons.dev/icons?i=mysql,postgres,mongodb" />
</p>

#### 🧰 Tools
<p>
  <img src="https://skillicons.dev/icons?i=git,github,figma,vscode" />
</p>

---

## 💼 Featured Projects (Hover Card Effect)

<div align="center" style="display:flex; gap:20px; flex-wrap:wrap; justify-content:center;">

<div style="border-radius:10px; box-shadow:0 5px 20px rgba(0,0,0,0.2); padding:15px; max-width:250px; transition:0.3s; text-align:left;">
**✈️ Flight Booking**  
[Live Demo](https://responsive-flight-agency-website.vercel.app/)  
Built with **React, TailwindCSS, Vite**.  
Responsive design & search flow.
</div>

<div style="border-radius:10px; box-shadow:0 5px 20px rgba(0,0,0,0.2); padding:15px; max-width:250px; transition:0.3s; text-align:left;">
**💻 Portfolio**  
[Live Demo](https://portfolio-q-plum.vercel.app/)  
Modern portfolio with animated sections & responsive UI.
</div>

<div style="border-radius:10px; box-shadow:0 5px 20px rgba(0,0,0,0.2); padding:15px; max-width:250px; transition:0.3s; text-align:left;">
**📚 JobHunt Pro**  
Coming Soon (Full-Stack MERN)
</div>

</div>

---

## 📊 GitHub Stats (Collapsible)

<details>
<summary>Click to Expand 📈</summary>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Sneara0&show_icons=true&theme=radical&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Sneara0&theme=radical&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs?username=Sneara0&show_icons=true&layout=compact&theme=radical&hide_border=true" />
</p>

</details>

---

## ✨ Skills Summary

```bash
Frontend : ⚛️ React, JavaScript, TypeScript, TailwindCSS, HTML, CSS
Backend  : Node.js, Express, Django, Next.js
Database : MongoDB, MySQL, PostgreSQL
Tools    : Git, GitHub, Figma, Illustrator, VS Code



---

### ✨ Skills Summary

```bash
Frontend : React, JavaScript, TypeScript, TailwindCSS, HTML, CSS
Backend  : Node.js, Express, Django, Next.js
Database : MongoDB, MySQL, PostgreSQL
Tools    : Git, GitHub, Figma, Illustrator, VS Code
"C:\Users\USER\Desktop\sneara..jfi-![IMG-20250727-WA0001](https://github.com/user-attachments/assets/c73f7237-3d57-476e-a08e-40f6da03d6c4c
